...
BYPASS_OLD_LOGPROB=True  # Use rollout log-probs as old_log_probs (Seq-Bypass mode)
...
generate_suffix() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      --bypass_old_logprob) suffix+="_bypass$2"; shift 2 ;;
      *) shift ;;
    esac
  done
...
while [[ "$#" -gt 0 ]]; do
  ...
  case "$1" in
    ...
    --val_start_batch) VAL_START_BATCH="$2"; shift 2 ;;
    ...
    --bypass_old_logprob) BYPASS_OLD_LOGPROB="$2"; shift 2 ;;
    *)
...
echo "Bypass Old LogProb: $BYPASS_OLD_LOGPROB"
...
PYTHONUNBUFFERED=1 python -m recipe.simpletir.main_simpletir \
    ...
    data.val_start_batch=$VAL_START_BATCH \
    ...
    actor_rollout_ref.actor.bypass_old_logprob=$BYPASS_OLD_LOGPROB \
