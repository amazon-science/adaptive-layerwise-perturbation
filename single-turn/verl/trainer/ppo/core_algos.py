...
# =============================================================================
# Perturbation Support Functions (from mismatch_rl)
# =============================================================================


def compute_ppo_is_metrics(log_importance_ratio, response_mask, advantages, clip_ratio_low, clip_ratio_high):
    """Compute PPO importance sampling metrics.
    
    Args:
        log_importance_ratio: Log of importance ratios, shape (batch_size, response_length)
        ...
        clip_ratio_low: Lower clip ratio
        clip_ratio_high: Upper clip ratio
    ...
    importance_ratios = torch.exp(log_importance_ratio)
...
def compute_policy_loss_perturbed(
    ...
    Args:
        old_log_prob (torch.Tensor):
            Log-probabilities of actions under the old policy, shape (batch_size, response_length).
            Note: This parameter is kept for compatibility but not used in the main calculation.
        log_prob (torch.Tensor):
            Log-probabilities of actions under the current policy (with perturbation), 
        ...
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Default: "token-mean".
        ...
        turn_end_indicator (torch.Tensor, optional):
            Turn end indicator, shape (batch_size, response_length). Required for "cum-turn" mode.
        ...
        config:
            Algorithm configuration object containing clip_ratio_low, clip_ratio_high, clip_ratio_c, etc.
            
    Returns:
        Tuple of (pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, ppo_is_metrics)
    """
    assert config is not None, "config is required for compute_policy_loss_perturbed"
    assert rollout_log_probs is not None, "rollout_log_probs is required when using perturbation"
    
    # Extract clip ratios from config
    ...
    # Handle void turn masking if applicable
    ...
    # Key difference: use rollout_log_probs as reference instead of old_log_prob
    # This accounts for the distribution shift caused by perturbation
    ...
    # Clamp log_importance_ratio for numerical stability
    log_importance_ratio = torch.clamp(log_importance_ratio, min=-20.0, max=20.0)
        
    # Calculate importance ratios (only for valid positions)
