#!/bin/bash
# Docker wrapper for bypass experiment
# Usage: bash docker/run_bypass.sh [loss_mode] [dataset]
set -e
...
docker run --rm \
    ...
    -e RAY_enable_auto_cleanup=true \
    -e RAY_memory_monitor_refresh_ms=1000 \
    ...
    bash -c "
        ...
        # Clean up Ray temporary files and caches
        echo '清理 Ray 缓存...'
        rm -rf /workspace/project/.cache/ray_tmp_8gpu/* 2>/dev/null || true
        rm -rf /workspace/project/.cache/tmp/* 2>/dev/null || true
        find /workspace/project/.cache -name '.nfs*' -type f -delete 2>/dev/null || true
        echo 'Ray 缓存已清理'
        ...
        bash run_exp_bypass.sh ${LOSS_MODE} ${DATASET}
    "
